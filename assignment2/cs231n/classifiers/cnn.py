...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    oH = H - filter_size + 1
    oW = W - filter_size + 1
    odim = oH * oW * num_filters / 2 / 2
    self.params['W1'] = np.random.normal(0, weight_scale, (num_filters, C, filter_size, filter_size))
    self.params['b1'] = np.zeros((num_filters,))
    self.params['W2'] = np.random.normal(0, weight_scale, (odim, hidden_dim))
  ...
  def loss(self, X, y=None):
    ...
    dscores /= N
    dout, dW3, db3 = affine_backward(dscores, cache3)
    dW3 += self.reg * W3
    dout, dW2, db2 = affine_relu_backward(dout, cache2)
    dW2 += self.reg * W2
    _, dW1, db1 = conv_relu_pool_backward(dout, cache1)
    dW1 += self.reg * W1
