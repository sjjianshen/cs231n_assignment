...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    loss = -np.sum(np.log(probs[np.arange(size), y]))
    loss /= size
    loss += 0.5 * self.reg * (np.sum(W1 * W1) + np.sum(W2 * W2))
    dscores = probs.copy()
    dscores[np.arange(size), y] -= 1
    dscores /= size
    dW2 = X1.T.dot(dscores)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in np.arange(self.num_layers):
        if i == 0:
            self.params['W%d' % (i + 1)] = np.random.normal(0, weight_scale,(input_dim, hidden_dims[i]))
            ...
            self.params['W%d' % (i + 1)] = np.random.normal(0, weight_scale,(hidden_dims[i-1], num_classes))
  ...
  def loss(self, X, y=None):
    ...
    for i in np.arange(self.num_layers):
        if i == self.num_layers - 1:
            scores, cache = affine_forward(input, self.params['W%d' % (i + 1)], self.params['b%d' % (i + 1)])
            caches.append(cache)
        else:
            input, cache1 = affine_forward(input, self.params['W%d' % (i + 1)], self.params['b%d' % (i + 1)])
            if self.use_batchnorm:
                input, cache2 = batchnorm_forward(input, self.params['gamma%d' %(  i + 1 )], self.params['beta%d' % (i + 1)], self.bn_params[i])
            else:
                cache2= None
            input, cache3 = relu_forward(input)
            if self.use_dropout:
                input, cache4 = dropout_forward(input, self.dropout_param)
            else:
                cache4 = None
            caches.append((cache1, cache2, cache3, cache4))
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for i in np.arange(self.num_layers-1, -1, -1):
        if i == self.num_layers - 1:
            ...
            cache1, cache2, cache3, cache4 = caches[i]
            if self.use_dropout:
                dout = dropout_backward(dout, cache4)
            dout = relu_backward(dout, cache3)
            if self.use_batchnorm:
                dout, dgamma, dbeta = batchnorm_backward(dout, cache2)
