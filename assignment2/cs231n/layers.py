...
def relu_backward(dout, cache):
  ...
  #############################################################################
  dx = dout * np.array(x > 0, dtype=int)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
...
def batchnorm_backward_alt(dout, cache):
  ...
  if mode == 'train':
    ...
    dbeta = dout.sum(axis=0)
    dgamma = np.sum(xn * dout, axis=0)
    dx = (np.sqrt(N) * xc * (xc - np.sum(xc, axis=0) / N) - (N - 1) * std) / (N * (std ** 2)) * dout * gamma
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  for i in np.arange(N):
      for (ih, iw) in [(ih, iw) for ih in np.arange(Hn) for iw in np.arange(Wn)]:
          x_start = ih * stride
          y_start = iw * stride
          x_end = x_start + HH
          y_end = y_start + WW
          for f in np.arange(F):
            out[i, f, ih, iw] = np.sum(w[f, :, :, :] * padded_x[i, :, x_start:x_end, y_start:y_end]) + b[f]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b, conv_param)
...
def conv_backward_naive(dout, cache):
  ...
  for i in np.arange(N):
      for (ih, iw) in [(ih, iw) for ih in np.arange(Hn) for iw in np.arange(Wn)]:
          ...
          y_end = y_start + WW
          for f in np.arange(F):
              dx[i, :, x_start:x_end, y_start:y_end] += w[f, :, :, :] * dout[i, f, ih, iw]
              dw[f, :, :, :] += dout[i, f, ih, iw] * padded_x[i, :, x_start:x_end, y_start:y_end]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def max_pool_forward_naive(x, pool_param):
  ...
  oH = 1 + ( H - pool_height ) / stride
  oW = 1 + ( W - pool_width ) / stride
  out = np.zeros((N, C, oH, oW))
  for (ih, iw) in [(ih, iw) for ih in np.arange(oH) for iw in np.arange(oW)]:
      x_start = ih * stride
      y_start = iw * stride
      x_end = x_start + pool_height
      y_end = y_start + pool_width
      out[:,:,ih,iw] = np.max(x[:,:,x_start:x_end,y_start:y_end], axis=(2,3))
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
  ...
  for (ih, iw) in [(ih, iw) for ih in np.arange(oH) for iw in np.arange(oW)]:
      ...
      for iN in np.arange(N):
          for iC in np.arange(C):
              patch_x = x[iN,iC,x_start:x_end,y_start:y_end]
              idx = np.argmax(patch_x)
              ydx = idx % pool_width
              xdx = idx / pool_width
              dx[iN,iC,x_start + xdx, y_start + ydx] += dout[iN,iC,ih,iw]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
...
def spatial_batchnorm_backward(dout, cache):
  ...
  if mode == 'train':
      ...
      dx = dxn
      dxc = dxn / std
      dstd = -np.sum(xc * dxn / (std * std), axis=(0, 2, 3), keepdims=True)
      dvar = dstd * 0.5 / std
      dxc2 = dvar * 2.0 / n * xc
      dxc += dxc2
      dx = dxc
      dmu = -np.sum(dx, axis=(0, 2, 3),keepdims=True)
      dx2 = dmu / n
      dx += dx2
